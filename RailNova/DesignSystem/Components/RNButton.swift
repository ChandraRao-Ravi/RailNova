...
struct RNPrimaryButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .frame(height: 52)
            .background(isDisabled ? Color.gray.opacity(0.4) : Color.rnSecondary)
...
struct RNSecondaryButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .frame(maxWidth: .infinity)
            .frame(height: 52)
            .background(Color.rnSecondary.opacity(0.12))
            .foregroundColor(.rnSecondary)
            .cornerRadius(12)
            .overlay(
                RoundedRectangle(cornerRadius: 12)
                    .stroke(Color.rnSecondary.opacity(0.3), lineWidth: 1)
            )
        }
    }
}
...
struct AvailabilityBadge: View {
    ...
    var statusColor: Color {
        switch status {
        case .available: return .rnAvailable
        case .waitlist: return .rnWaitlist
        case .rac: return .rnRAC
        ...
        Button(action: onTap) {
            VStack(spacing: 4) {
                Text(travelClass.rawValue)
                    .font(RNTypography.labelLarge)
                    .foregroundColor(isSelected ? .white : .rnTextPrimary)
                Text(status.displayText)
                    .font(RNTypography.labelSmall)
                    .foregroundColor(isSelected ? .white.opacity(0.9) : statusColor)
                Text("₹\(Int(fare))")
                    .font(RNTypography.labelMedium)
                    .foregroundColor(isSelected ? .white : .rnTextSecondary)
            }
            .padding(.horizontal, 12)
            .padding(.vertical, 8)
            .background(isSelected ? Color.rnSecondary : Color.rnCardBackground)
            .cornerRadius(8)
            .overlay(
